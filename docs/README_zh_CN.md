...
- **[nvim-treesitter](https://github.com/nvim-treesitter/nvim-treesitter)**：Java 和 XML 解析需要它。nvim-mybatis 会将自定义的 `tree-sitter-mybatis` 注册到 nvim-treesitter，并自动安装这三个解析器。匹配的 Mapper XML 文件使用 `mybatis.xml` 混合文件类型和 MyBatis 解析器。XML 语言服务器配置应同时包含 `xml` 和 `mybatis.xml` 文件类型。
